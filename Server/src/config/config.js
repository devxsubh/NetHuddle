...
const envValidate = Joi.object()
	.keys({
		...
		TCP_PORT: Joi.number().allow('').empty('').default(3001),
		QUIC_SERVER_HOST: Joi.string().allow('').empty('').default('localhost'),
		QUIC_SERVER_PORT: Joi.number().allow('').empty('').default(4433)
	})
	.unknown();
...
export default {
	...
	QUIC_SERVER_HOST: env.QUIC_SERVER_HOST,
	QUIC_SERVER_PORT: env.QUIC_SERVER_PORT,
